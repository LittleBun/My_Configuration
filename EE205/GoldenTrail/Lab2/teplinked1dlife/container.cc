...
{
	if(p ==END) p = size();
	if( !valid_position(p, count-1)){
		Warning("Container: Remove a illegal position; nothing done");
		return ERROR;
	}
	if(p == 0){
		Entry data = head->getdata();
		head = findpos(p+1);
		count --;
		return data;
	}
	ListNode *current = findpos(p-1);
	Entry data = current->getnext()->getdata();
	current->setnext(findpos(p+1));
	count --;
	return data;
}
